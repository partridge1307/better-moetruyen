...
interface MangaImageFormProps {
  form: UseFormReturn<MangaUploadPayload>;
}

const MangaImageForm: FC<MangaImageFormProps> = ({ form }) => {
  return (
    <FormField
          ...
          <FormControl>
            <AspectRatio ratio={4 / 3}>
              {!!field.value ? (
                ...
                <div
                  role="button"
                  className="w-full h-full relative flex justify-center items-center hover:cursor-pointer rounded-md border-2 dark:border-zinc-800"
                  onClick={(e) => {
                    e.preventDefault();
